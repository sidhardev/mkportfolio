document.addEventListener('DOMContentLoaded', () => {
    ...
    function generatePortfolioHTML(data) {
        // Default styles (previously configurable)
        const bodyFontFamily = "'Inter', sans-serif";
        const layoutSpacingClasses = "max-w-6xl mx-auto px-4 py-12";
        const cardClasses = "rounded-xl shadow-lg";
        const aosDataAttribute = "fade-up"; // Default AOS animation type
        const aosDuration = "1000"; // Default AOS animation duration
        const primaryColorClass = "text-blue-600"; // For text, links (e.g., Tailwind's blue-600)
        const primaryHoverColorClass = "hover:text-blue-700"; // Hover state for links
        const skillTagClasses = "bg-blue-100 text-blue-800 px-3 py-1.5 rounded-full text-sm font-medium"; // Classes for skill tags
        ...
        const projectsHTML = data.projects.map(project => `
            <div class="bg-white p-6 ${cardClasses} mb-6" data-aos="${aosDataAttribute}" data-aos-duration="${aosDuration}">
                ...
                <div class="mt-4 flex space-x-3">
                    ${project.liveUrl ? `<a href="${project.liveUrl}" target="_blank" rel="noopener noreferrer" class="${primaryColorClass} ${primaryHoverColorClass} hover:underline text-sm font-medium">Live Demo</a>` : ''}
                    ${project.repoUrl ? `<a href="${project.repoUrl}" target="_blank" rel="noopener noreferrer" class="${primaryColorClass} ${primaryHoverColorClass} hover:underline text-sm font-medium">Source Code</a>` : ''}
                </div>
            </div>
        `).join('');
        ...
        return `
            ...
            <body class="text-gray-800" onload="AOS.init();">
                <div class="${layoutSpacingClasses}">
                    <header class="text-center mb-10 md:mb-12" data-aos="${aosDataAttribute}" data-aos-duration="${aosDuration}">
                        ...
                        ${data.tagline ? `<p class="text-lg md:text-xl ${primaryColorClass} font-medium mt-1">${data.tagline}</p>` : ''}
                    </header>

                    ${data.bio ? `
                    <section id="about" class="mb-8 md:mb-10 p-6 bg-white ${cardClasses}" data-aos="${aosDataAttribute}" data-aos-duration="${aosDuration}">
                    ...
                    <section id="contact" class="mb-8 md:mb-10 p-6 bg-white ${cardClasses}" data-aos="${aosDataAttribute}" data-aos-duration="${aosDuration}">
                        <h2 class="text-2xl font-semibold text-gray-700 mb-3 border-b pb-2">Contact</h2>
                        <ul class="list-none p-0 space-y-2">
                            ${data.email ? `<li><strong class="font-medium">Email:</strong> <a href="mailto:${data.email}" class="${primaryColorClass} ${primaryHoverColorClass} hover:underline">${data.email}</a></li>` : ''}
                            ${data.linkedin ? `<li><strong class="font-medium">LinkedIn:</strong> <a href="${data.linkedin}" target="_blank" rel="noopener noreferrer" class="${primaryColorClass} ${primaryHoverColorClass} hover:underline">${data.linkedin.replace(/^(https?:\/\/)?(www\.)?/, '')}</a></li>` : ''}
                            ${data.github ? `<li><strong class="font-medium">GitHub:</strong> <a href="${data.github}" target="_blank" rel="noopener noreferrer" class="${primaryColorClass} ${primaryHoverColorClass} hover:underline">${data.github.replace(/^(https?:\/\/)?(www\.)?/, '')}</a></li>` : ''}
                        </ul>
                    </section>
                    ` : ''}

                    ${data.skills && data.skills.length > 0 ? `
                    <section id="skills" class="mb-8 md:mb-10 p-6 bg-white ${cardClasses}" data-aos="${aosDataAttribute}" data-aos-duration="${aosDuration}">
                        <h2 class="text-2xl font-semibold text-gray-700 mb-4 border-b pb-2">Skills</h2>
                        <div class="flex flex-wrap gap-2">
                            ${data.skills.map(skill => `<span class="${skillTagClasses}">${skill}</span>`).join('')}
                        </div>
                    </section>
                    ` : ''}
                    ...
                    <section id="projects">
                        <h2 class="text-2xl font-semibold text-gray-700 mb-6 border-b pb-2" data-aos="${aosDataAttribute}" data-aos-duration="${aosDuration}">Projects</h2>
                    ...
                    <footer class="text-center mt-10 md:mt-16 py-6 border-t border-gray-200" data-aos="${aosDataAttribute}" data-aos-duration="${aosDuration}">
